...
2. The user opens the emailed link → `/auth/callback`. The callback supports two flows: `token_hash`+`type` via `verifyOtp` (the recommended server-side flow — works regardless of which browser/device opens the link), and `code` via `exchangeCodeForSession` (PKCE; same-browser only) as a fallback. Either sets the session cookies (server client) → redirect to `/` (relative-only, no open redirect). The Supabase email templates point at `{{ .SiteURL }}/auth/callback?token_hash={{ .TokenHash }}&type=email`.
...
- `rate-limit.ts` reclaims abandoned buckets with an O(n) sweep that is now throttled to at most once per `CLEANUP_INTERVAL_MS` (60s); per-key expiry is still enforced lazily on access, so the limit itself is unaffected between sweeps.
...
| 2026-06-23 | Throttled the rate-limiter's `cleanupExpired` sweep to at most once per 60s (`CLEANUP_INTERVAL_MS`) instead of running the O(n) map scan on every request; per-key expiry is still enforced lazily on access so the limit is unchanged. Added an injectable `now` param plus `resetRateLimiter`/`getActiveBucketCount` test helpers and throttle/window regression tests. | `src/lib/api/rate-limit.ts`, `src/lib/api/rate-limit.test.ts`, `architecture.md` |
...
| 2026-06-23 | `provider-fli.healthCheck` now reports the real probe round-trip latency (measured inside `getFliHealth` and memoized with the result) instead of the near-zero time to read the health cache — a warm-instance health check shows true latency. Removed from Known Limitations. | `src/lib/providers/provider-fli.ts`, `architecture.md` |
| 2026-06-23 | `/api/search` now declares `maxDuration = 60` (Vercel Hobby ceiling) so a cold live fan-out isn't killed by the platform default — same `maxDuration` mechanism the digest route uses (digest sets 300; search uses 60 as a user-facing route). Complements the `FLI_MAX_DESTINATIONS` fan-out cap. | `src/app/api/search/route.ts`, `architecture.md` |
