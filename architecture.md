...
| 2026-06-23 | `/api/search` now declares `maxDuration = 60` (Vercel Hobby ceiling) so a cold live fan-out isn't killed by the platform default; mirrors the digest route. Complements the `FLI_MAX_DESTINATIONS` fan-out cap. | `src/app/api/search/route.ts`, `architecture.md` |
